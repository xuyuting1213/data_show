  ...
  <div>
    ...
    <div class="table-container">
      ...
      >
        ...
        >
          <template #default="scope">
            <!-- <el-button
              ...
              type="text"
              size="small"
              style="color: red; font-size: 0.175rem; margin-top: -0.1rem"
            >
              删除
            </el-button> -->
            <el-button
              ...
              type="text"
              size="small"
              style="font-size: 0.175rem; margin-top: -0.1rem"
            >
              编辑
            </el-button>
          </template>
        ...
        <el-table-column
          fit
          v-for="(column, index) in columns.slice(0, 7)"
          :key="index"
          :prop="column.prop"
          :label="column.label"
        ...
        <el-table-column
          fit
          :key="'熔融金属紧急排放和应急储存设施配置'"
          :prop="'熔融金属紧急排放和应急储存设施配置'"
          :label="'熔融金属紧急排放和应急储存设施配置'"
        >
          <el-table-column
            :prop="'中间罐溢流坑（槽）'"
            :label="'中间罐溢流坑（槽）'"
          >
            ...
            >
              <template #default="scope">
                <div
                  :class="
                    scope.row['中间罐溢流坑（槽）数量（个）图片']
                      ? 'specialCell'
                      : ''
                  "
                >
                  {{ scope.row["中间罐溢流坑（槽）数量（个）"] }}
                </div>
                <img
                  v-if="scope.row['中间罐溢流坑（槽）数量（个）图片']"
                  @click="showbigimg(imgs)"
                  v-for="imgs in String(
                    scope.row['中间罐溢流坑（槽）数量（个）图片']
                  ).split('|')"
                  :key="imgs"
                  :src="imgs"
                  style="width: 1rem; margin-bottom: 0.1rem"
                />
              </template>
              <!-- <template #default="scope">
                <div
                ...
                <el-button size="small" type="text" @click="showcellimg(scope,'中间罐溢流坑（槽）数量（个）图片')">点击查看图片</el-button>
              {{ scope.row.imgsource }}aaa
                <img
                  @click="showbigimg(imgs)"
                  v-if="scope.row.imgsource&&scope.row.imgsource.length"
                  v-for="imgs in scope.row.imgsource.split('|')"
                  :key="imgs"
                  :src="imgs"
                  style="width: 1rem; margin-bottom: 0.1rem;"
                />
          ...
          <el-table-column :prop="'漏钢回转溜槽'" :label="'漏钢回转溜槽'">
            <el-table-column
              :prop="'漏钢回转溜槽数量（个）'"
              :label="'数量（个）'"
            >
              <template #default="scope">
                <div
                  :class="
                    scope.row['漏钢回转溜槽数量（个）图片'] ? 'specialCell' : ''
                  "
                >
                  {{ scope.row["漏钢回转溜槽数量（个）"] }}
                </div>
                <img
                  @click="showbigimg(imgs)"
                  v-if="scope.row['漏钢回转溜槽数量（个）图片']"
                  v-for="imgs in String(
                    scope.row['漏钢回转溜槽数量（个）图片']
                  ).split('|')"
                  :key="imgs"
                  :src="imgs"
                  style="width: 1rem; margin-bottom: 0.1rem"
          ...
          <el-table-column :prop="'事故钢水罐配置'" :label="'事故钢水罐配置'">
            <el-table-column
              :prop="'事故钢水罐配置有效容积（m3）'"
              :label="'有效容积（m3）'"
            >
              <template #default="scope">
                <div
                  :class="
                    scope.row['事故钢水罐配置有效容积（m3）图片']
                      ? 'specialCell'
                      : ''
                  "
                >
                  {{ scope.row["事故钢水罐配置有效容积（m3）"] }}
                </div>
                <img
                  @click="showbigimg(imgs)"
                  v-if="scope.row['事故钢水罐配置有效容积（m3）图片']"
                  v-for="imgs in String(
                    scope.row['事故钢水罐配置有效容积（m3）图片']
                  ).split('|')"
                  :key="imgs"
            ...
            >
              <template #default="scope">
                <div
                  :class="
                    scope.row['事故钢水罐配置额定盛装量（t）图片']
                      ? 'specialCell'
                      : ''
                  "
                >
                  {{ scope.row["事故钢水罐配置额定盛装量（t）"] }}
                </div>
                <img
                  @click="showbigimg(imgs)"
                  v-if="scope.row['事故钢水罐配置额定盛装量（t）图片']"
                  v-for="imgs in String(
                    scope.row['事故钢水罐配置额定盛装量（t）图片']
                  ).split('|')"
                  :key="imgs"
          ...
          >
            ...
            >
              <template #default="scope">
                <div
                  :class="
                    scope.row['中间罐漏钢坑（槽）数量（个）图片']
                      ? 'specialCell'
                      : ''
                  "
                >
                  {{ scope.row["中间罐漏钢坑（槽）数量（个）"] }}
                </div>
                <img
                  @click="showbigimg(imgs)"
                  v-if="scope.row['中间罐漏钢坑（槽）数量（个）图片']"
                  v-for="imgs in String(
                    scope.row['中间罐漏钢坑（槽）数量（个）图片']
                  ).split('|')"
                  :key="imgs"
            ...
            <el-table-column
              :prop="'中间罐漏钢坑（槽）有效容积（m3）'"
              :label="'有效容积（m3）'"
            >
              <template #default="scope">
                <div
                  :class="
                    scope.row['中间罐漏钢坑（槽）有效容积（m3）图片']
                      ? 'specialCell'
                      : ''
                  "
                >
                  {{ scope.row["中间罐漏钢坑（槽）有效容积（m3）"] }}
                </div>
                <img
                  @click="showbigimg(imgs)"
                  v-if="scope.row['中间罐漏钢坑（槽）有效容积（m3）图片']"
                  v-for="imgs in String(
                    scope.row['中间罐漏钢坑（槽）有效容积（m3）图片']
                  ).split('|')"
                  :key="imgs"
            ...
            <el-table-column
              :prop="'中间罐漏钢坑（槽）额定盛装量（t）'"
              :label="'额定盛装量（t）'"
            >
              <template #default="scope">
                <div
                  :class="
                    scope.row['中间罐漏钢坑（槽）额定盛装量（t）图片']
                      ? 'specialCell'
                      : ''
                  "
                >
                  {{ scope.row["中间罐漏钢坑（槽）额定盛装量（t）"] }}
                </div>
                <img
                  @click="showbigimg(imgs)"
                  v-if="scope.row['中间罐漏钢坑（槽）额定盛装量（t）图片']"
                  v-for="imgs in String(
                    scope.row['中间罐漏钢坑（槽）额定盛装量（t）图片']
                  ).split('|')"
                  :key="imgs"
      ...
      <el-button
        ...
        @click="
          $downloadExcel({
            tableData: tabledata,
            title: '炼钢（连铸）生产过程熔融金属紧急排放和应急储存设施',
          })
        "
        class="plusicon"
        size="small"
      ...
      <div class="dialog-content">
        ...
        >
          ...
          >
            ...
            <div v-if="column.prop.indexOf('图片') >= 0">
              ...
              <Uploadocom
                ...
                ref="uploadocom"
                :filelistdata="newRow[column.prop] ? newRow[column.prop] : []"
              ></Uploadocom>
              <div>
                <div
                  v-for="imgs in newRow[column.prop]"
                  style="width: 1rem; float: left; margin-right: 0.1rem"
                >
                  <img
                    @click="showbigimg(imgs)"
                    v-if="newRow[column.prop].length"
                    :key="imgs"
                    :src="imgs"
                    style="width: 1rem; height: 1rem; margin: 0.1rem 0.1rem 0 0"
                  />
                  <div style="width: 1rem">
                    <span
                      style="color: #247cc3; cursor: pointer"
                      @click="cancelImg(column.prop, imgs)"
                      >取消</span
                    >
                  </div>
                </div>
            ...
            <el-input
              v-else-if="
                column.prop.indexOf('图片') < 0 && column.prop != '运行状态'
              "
              v-model="newRow[column.prop]"
    ...
    >
      <div style="width: 100%">
        <img @click="showbigimg(imgs)" :src="currimgsrc" style="width: 100%" />
      </div>
    </el-dialog>
...
import Uploadocom from "../../components/uploadlist.vue";
...
export default {
  ...
  methods: {
    ...
    showcellimg(scope, name) {
      //  row.imgsource=row[scope.$index]
      this.$set(this.paginatedData[scope.$index], "imgsource", scope.row[name]);
      this.$set(scope.row, "imgsource", scope.row[name]);
      console.log(this.paginatedData[scope.$index]);
      this.$forceUpdate();
    },
    ...
    cancelImg(column, img) {
      // if(this.d)
      this.newRow[column] = this.newRow[column].filter((item) => item != img);
    },
    imageshowFunc(msg, column) {
      this.newRow[column] = msg;
      console.log("图", this.newRow[column]);
    },
    showbigimg(src) {
      this.imgdialog = true;
      this.currimgsrc = src;
    ...
    formatRow() {
      ...
      this.newRow = this.columns.reduce((obj, column) => {
        obj[column.prop] = column.prop.indexOf("图片") >= 0 ? [] : "";
        return obj;
      }, {});
    },
  },
