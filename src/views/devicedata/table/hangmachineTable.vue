  ...
  <div>
    ...
    <div class="table-container" :style="{ height: heightValue }">
          ...
          <template #default="scope">
            <div v-if="!scope.row.editing">
              ...
              >
                ...
                <span v-else>
                  <span v-if="column.prop == '起重机品种'">
                    {{ formatShowData(scope.row[column.prop], "cranetype") }}
                  </span>
                  <span v-if="column.prop == '起重机类型'">
                    {{
                      formatShowData(
                        scope.row[column.prop],
                        "craneFunctiontype"
                  ...
                  <span
                    v-if="
                      column.prop != '起重机类型' && column.prop != '起重机品种'
                    "
                  >
     
                    <span v-if="column.prop == '检验类型'">
                      {{
                        formatShowData(scope.row[column.prop], "deviceKind2")
                      }}
                    </span>
                    <span v-else>
                      {{ scope.row[column.prop] }}
      ...
      <el-button
        type="primary"
        @click="
          $downloadExcel({ tableData: tabledata, title: '吊运熔融金属起重机' })
        "
        class="plusicon"
        size="small"
        >导出数据</el-button
    ...
    >
      <div class="dialog-content">
        ...
        >
          <el-form-item
            v-for="(column, coindex) in columns"
            :key="column.prop"
            :label="column.label"
            :prop="column.prop"
            v-if="
          ...
          >
            ...
            <el-select
              v-if="column.prop == '起重机品种'"
              v-model="newRow[column.prop]"
              style="width: 8rem"
            >
              <el-option
                v-for="item in cranetype"
                :label="item.label"
                :value="item.value"
                :key="item.label"
              ></el-option>
            ...
            <el-select
              v-if="column.prop == '起重机类型'"
              v-model="newRow[column.prop]"
              style="width: 8rem"
            >
              <el-option
                v-for="item in craneFunctiontype"
                :label="item.label"
                :value="item.value"
                :key="item.label"
              ></el-option>
            ...
            <el-select
              ...
              "
            >
              <el-option v-for="item in deviceKind2" :label="item.label" :value="item.value"
                                :key="item.label"></el-option>
            </el-select>

            <div v-if="column.prop == '检验报告上传'">
              <Uploadocom
                v-if="
                  newRow[column.prop] === '' &&
                  !(
...
export default {
  props: ["column", "tabledata", "operate", "cranetype", "craneFunctiontype"],
