...
import BaseTable from "./table/gasboxTable.vue";
...
export default {
  ...
  methods: {
    ...
    async editinfo(info, base_pa) {
      ...
      if (info.operate == "编辑") {
        ...
        apifunc = api.updateGasbox({ ...params });
      } else {
        ...
        // console.log(params);
        apifunc = api.writeGasbox({ ...params });
      }
    ...
    getdata(params) {
      ...
      api
        .readGasbox(params ? { filter: JSON.stringify({ ...params }) } : "")
        .then((res) => {
  ...
  mounted() {
    this.getdata("");
    this.gtType = this.gtType.map((item, index) => {
      return {
        label: item,
