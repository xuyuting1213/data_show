<template>
  <div class="deviceinfocontainer intor_list highlight">
    ...
    <BaseTable
      ...
      :gpdShutoffType="gpdShutoffType"
      :gpdGoggleType="gpdGoggleType"
      :gpdGoggleDrivemode="gpdGoggleDrivemode"
    ></BaseTable>
...
import BaseTable from "./table/glassgasdeviceTable.vue";
export default {
  ...
  data() {
    return {
      ...
      current_newinfo: {},
      filter_params: {},
      gpdShutoffType: ["蝶阀", "闸阀", "球阀", "其他"],
      gpdGoggleType: ["扇形", "敞开式", "全封闭式", "无"],
      gpdGoggleDrivemode: ["电动", "液压", "气动"],
    };
  },
  ...
  methods: {
    ...
    async editinfo(info, base_pa) {
      ...
      let params = {},
        apifunc;
      base_pa[0]["设备类型"] = "煤气设施（管道）眼镜阀组合式隔断装置";
      if (info.operate == "编辑") {
        params = {
        ...
        apifunc = api.updateGaspartitiondevice({ ...params });
      } else {
        ...
        apifunc = api.writeGaspartitiondevice({ ...params });
      }
  ...
  mounted() {
    this.getdata("");
    this.gpdShutoffType = this.gpdShutoffType.map((item, index) => {
      return {
        label: item,
        value: (index + 1).toString(),
      };
    });
    this.gpdGoggleType = this.gpdGoggleType.map((item, index) => {
      return {
        label: item,
        value: (index + 1).toString(),
      };
    });
    this.gpdGoggleDrivemode = this.gpdGoggleDrivemode.map((item, index) => {
      return {
        label: item,
        value: (index + 1).toString(),
      };
