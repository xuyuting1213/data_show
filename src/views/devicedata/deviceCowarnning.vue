...
import BaseTable from "./table/coTable.vue";
...
export default {
  ...
  methods: {
    ...
    addinfo(info) {
      ...
      let speci_pa = this.$assignValues("一氧化碳报警器", base_pa[1]);
    ...
    async editinfo(info, base_pa, speci_pa) {
      ...
      if (info.operate == "编辑") {
        ...
        apifunc = api.updateCoalarm({ ...params });
      } else {
        ...
        apifunc = api.writeCoalarm({ ...params });
      }
    ...
    getdata(params) {
      ...
      api
        .readCoalarm(params ? { filter: JSON.stringify({ ...params }) } : "")
        .then((res) => {
