<template>
  <div class="deviceinfocontainer intor_list highlight">
    ...
    <BaseTable
      ...
      :cranetype="cranetype"
      :craneFunctiontype="craneFunctiontype"
    ></BaseTable>
...
import BaseTable from "./table/hangmachineTable.vue";
export default {
  ...
  methods: {
    ...
    addinfo(info) {
      ...
      let speci_pa = this.$assignValues("吊运熔融", base_pa[1]);
    ...
    async editinfo(info, base_pa, speci_pa) {
      ...
      if (info.operate == "编辑") {
        ...
        apifunc = api.updateHangmachine({ ...params });
      } else {
        ...
        // console.log(params);
        apifunc = api.writeHangmachine({ ...params });
  ...
  mounted() {
    this.getdata();
    this.cranetype = this.cranetype.map((item, index) => {
      return {
        label: item,
        value: (index + 1).toString(),
      };
    });
    this.craneFunctiontype = this.craneFunctiontype.map((item, index) => {
      return {
        label: item,
        value: (index + 1).toString(),
      };
