<template>
      ...
      <div class="mb8">
        ...
        <el-row style="margin-top: 10px">
          <div style="float: left">
            <el-date-picker
              style="margin-left: 3px; width: 6rem"
  ...
  export default {
    ...
    watch: {
       newId: {
         ...
         handler(newVal, oldVal) {
            if (newVal) {
              this.myNewId = 'divPlayBack' + newVal;
            }
          },
       },
       data: {
          ...
          immediate: true,
          handler(newVal, oldVal) {
             console.log('backvideo:oldVal', oldVal);
             console.log('backvideo:newVal', newVal);
             if (newVal && newVal.hkData) {
               var hkData = newVal.hkData;
               this.ip = hkData.nvrAddress; //nvr地址
               this.myNewId = "divPlayBack" + newVal.id;
               this.userName = hkData.userName; //访问nvr的用户名
               this.passWord = hkData.passWord; //访问密码
               if (hkData.rtspPort) 
                  this.rtspPort = hkData.rtspPort;
               else this.rtspPort = 554; //默认的端口
               if (hkData.transStream) 
                  this.transStream = true;
               else 
                  this.transStream = false;
               if (hkData.port) 
                  this.port = hkData.port; //访问端口
               else this.port = 80; //80端口
               this.iChannelID = !hkData.channelID ? 1 : hkData.channelID; //通道ID
               if (hkData.title) 
                  this.title = hkData.title; //标题
               else 
                  this.title = "监控摄像头";
               if (hkData.streamType) 
                  this.streamType = hkData.streamType;
    ...
    mounted() {
      ...
      this.getTimeRange();
      if(this.ip){
         this.initS(); //本页面加载的时候执行初始化加载数据的操作
      }    
    },
    methods: {
      closeModal() {
      ...
      handlePlayBack(row) {
        ...
        // 设备标识
        const szDeviceIdentify = `${this.ip}_${this.port}`;
        const playbackURI = row.playbackURI; // 播放地址
        const szStartTime = row.startTime0; // 开始时间
        const szEndTime = row.endTime0; // 结束时间
  
        // 播放配置
        const iWndIndex = this.g_iWndIndex || 0; // 窗口索引
        const iRtspPort = this.rtspPort; // RTSP端口
        const iStreamType = this.streamType || 1; // 流类型
        const bZeroChannel = this.bZeroChannel; // 零通道标志
        const bTransStream = this.transStream; // 转码标志
  
        // 检查零通道
        if (bZeroChannel) {
          console.warn("当前为零通道，无法播放回放！");
          this.$message.warning("零通道不支持回放！");
          return;
        }
        ...
        const startPlayback = () => {
          const options = {
            ...
            success: () => {
              ...
              // this.$message.success("回放开始成功！");
            },
            error: (oError) => {
              console.error(
                `${szDeviceIdentify} 开始回放失败！`,
                oError.errorCode,
                oError.errorMsg
              );
              this.$message.error(`回放失败：${oError.errorMsg || "未知错误"}`);
            },
          };
  
          console.log("播放参数：", options);
        ...
        if (oWndInfo) {
          ...
          WebVideoCtrl.I_Stop({
            iWndIndex,
            success: () => {
              console.log("停止当前播放，准备开始新回放...");
              startPlayback();
            },
            error: (oError) => {
              console.error("停止播放失败：", oError.errorCode, oError.errorMsg);
              this.$message.error("停止当前播放失败，请重试！");
            },
          });
        } else {
          console.log("窗口空闲，直接开始播放...");
          startPlayback();
        }
      },
      ...
      async initS() {
        ...
        if (!this.plugInInitialized) {
          ...
          await WebVideoCtrl.I_InitPlugin({
            ...
            cbInitPluginComplete: function () {
              ...
              console.log('pluginitialized:', that.plugInInitialized);
  
              WebVideoCtrl.I_InsertOBJECTPlugin(that.myNewId).then(
                () => {
                  console.log("init ok.");
                  // 检查插件是否最新
        ...
        setTimeout(() => {
          ...
          this.setVideoWindowResize(this.myNewId, "500", "300");
          this.clickLogin();
        }, 500);
      },
          ...
          this.passWord,
          {
            ...
            error: function (err) {
              ...
              if (err.errorCode == 2001) {
                ...
                that.$message.error("设备登录失败，请检查登陆密码/访问地址的配置是否正确！");
              }
            },
          }
        );
        ...
        WebVideoCtrl.I_RecordSearch(
          ...
          szEndTime,
          {
            ...
            success: function (xmlDoc) {
              ...
              console.log(
                "responseStatusStrg",
                $(xmlDoc).find("responseStatusStrg")
              );
