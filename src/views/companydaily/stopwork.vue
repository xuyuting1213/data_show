...
import BaseTable from "./table/stopworkTable.vue";
  ...
  methods: {
    ...
    async reportfuncimginfo(selectval) {
      ...
      for (const item of report_images) {
        try {
          let res1 = await this.$reportimginfo(item, "5");
          if (res1.data&&res1.data.code != 2000)  {
            this.$message.error("停用证明资料上报失败");
          }
        } catch (error) {
    ...
    reportInfo(selectval) {
      ...
      if (ids.length) {
        this.loading = true;
        reportapi.stopreportDevice(params).then((res) => {
          ...
          if (res.data.code == 200) {
            ...
            res.data.data.map((item) => {
              console.log(item);
              let success_info =
                item.stopworkRecordMessage.code === 2000 ||
                item.stopworkRecordMessage.message.includes("表中数据已存在");
                  ...
                  .map(
                    (item) =>
                      `<div>ID:${item.id}_错误信息${
                        item.stopworkRecordMessage.code == 2000
                          ? ""
                          : item.stopworkRecordMessage.message
                      }</div>`
                  )
                  .join("")}</div>`,
                showClose: true,
    ...
    async editinfo(info, base_pa) {
      ...
      if (info.operate == "编辑") {
        params = {
          stopwork_data: JSON.stringify(Object.values(base_pa[1])),
          system_id: info.rowinfo.系统ID,
          record_id: info.rowinfo.ID,
        };
        apifunc = api.updateStopwork({ ...params });
      } else if (info.operate == "添加停用信息") {
        ...
        apifunc = api.writeStopwork({ ...params });
      }
    ...
    getdata() {
      ...
      api
        .readStopwork(params ? { filter: JSON.stringify({ ...params }) } : "")
        .then((res) => {
