    ...
    <div>
        ...
        > -->
            ...
            <el-col :span="18">
                ...
                <span style="color:white">仅满足以下条件的数据可上报：1、已经新增上报成功的设备 2、已经进行点检操作过的设备</span>
        ...
        <div class="table-container">
            <el-table bordered row-class-name="rowstyle" :data="paginatedData"
                ...
                <el-table-column fit v-for="(column, index) in column" :key="index" :prop="column.prop"
                    :label="column.label" :sortable="column.sortable" v-if="column.label != '系统ID'&& column.label != '设备编码'&&column.label != '点检事项'&&
              column.label != '点检序号'"    >
                    <template #default="scope">
        ...
        <el-dialog :title="operatetitle" :visible.sync="addRowDialogVisible" :modal="false" top="0.8rem"
            ...
            <div class="dialog-content">
                <el-form :model="newRow" label-position="top" ref="newRow" :rules="rules_data">
                    <el-form-item v-for="(column, coindex) in columns" :key="column.prop"
                        :label="column.label!='问题描述'?column.label:(newRow['点检结果']=='异常'?'问题描述':'')" :prop="column.prop"
                        v-if="
                        column.label != 'ID' &&
                        column.label != '系统ID' &&
                        column.prop != '检验日期预报警状态' &&
                        ...
                        <el-select v-if="column.prop == '点检标准'" v-model="newRow[column.prop]" style="width: 8rem">
                            <el-option v-for="item in deviceKind" :label="item" :value="item" :key="item"></el-option>
                        </el-select>
                        <el-select v-model="newRow[column.prop]" v-if="column.prop == '点检结果'" style="width: 8rem"
                            @change="changeStatus">
                            <el-option v-for="item in deviceKind2" :label="item" :value="item" :key="item"></el-option>
                        </el-select>
                        <div v-if="column.prop == '图片描述'">
                        ...
                        <el-input
                            v-else-if="column.prop != '运行状态' && column.prop != '图片描述'&&column.prop != '点检标准'&&column.prop != '点检结果'&&column.prop != '问题描述'"
                        ...
                        <el-input v-if="newRow['点检结果']=='异常'&&column.prop == '问题描述'" v-model="newRow[column.prop]"
                            :placeholder="'请输入' + column.label" style="width: 8rem"></el-input>
                    </el-form-item>
                </el-form>
            </div>
...
import historyDialog from "./historycheckDialog.vue";
...
export default {
    ...
    methods: {
            ...
            this.tableData.forEach(item => {
                if (item[prop]) {
                    values.add(item[prop]);
                }
            });
            return Array.from(values).map(value => ({ text: value, value }));
        },
        changeStatus(value) {
            // console.log(value); 
            if (value == '异常') {
                this.columns = []
                // 定义其他字段的规则...
                // this.rules_data["问题描述"] = [
                //     { required: false, message: '请输入问题描述', trigger: 'blur' }
                // ]
                this.rules_data["点检结果"] = [
                    { required: true, message: '请选择点检结果', trigger: 'change' },
                    {
                        validator: (rule, value, callback) => {
                            if (value === "异常") {
                                // 检查问题描述是否为空
                                if (!this.newRow["问题描述"] || this.newRow["问题描述"].trim() === "") {
                                    callback(new Error("当 '点检结果' 为 异常 时，问题描述 字段必须存在且非空！"));
                                } else {
                                    callback();
                                }
                            } else {
                ...
                this.columns = JSON.parse(JSON.stringify(this.column))
        ...
        openAddRowDialog(row) {
            ...
            this.$nextTick(() => {
                ...
                this.newRow["运行状态"] = "在用";
                this.newRow["点检结果"] = "正常";
                //   delete this.newRow['问题描述']
                this.rules_data = this.$formatFormRules([], this.newRow, {});
                console.log(this.rules_data);
                // delete this.rules_data['问题描述']
            });
        },
        addRow(formname) {
    ...
    computed: {
        paginatedData() {
            this.columns = JSON.parse(JSON.stringify(this.column));
            //    this.columns.splice(6, 7, {
            //     label:"点检结果",
            //     prop:"点检结果",
            //    })
            let ini_data = JSON.parse(JSON.stringify(this.tabledata))
            this.tableData = ini_data.map((item) => ({
                ...item,
            }));
    ...
    mounted() {
        // this.columns = this.columns.filter(item => item.prop != '问题描述')
