        ...
        >
          <template #default="scope">
            <div v-if="!scope.row.editing">
              <div
                v-if="
                  column.prop == '检验报告上传' || column.prop == '停用证明资料'
                "
...
import historyDialog from "./stopDialog.vue";
...
export default {
  ...
  methods: {
    formatShowData(row) {
      //   console.log(row);
      if (this.deviceKind.find((info) => info.value == row)) {
        return this.deviceKind.find((info) => info.value == row).label;
      } else {
        return row;
      }
    },
